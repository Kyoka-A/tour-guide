      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-20 reveal">
          ...
          <h2 className="text-4xl md:text-5xl font-black tracking-tight text-stone-900 mb-5">
            The <span className="text-gold-gradient">Experience</span>
          </h2>
        ...
        {/* タイムライン */}
        <div className="relative pl-10 md:pl-0 timeline-wrapper">
          {timeline.map((item, idx) => (
            <div
              key={idx}
              className="relative flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-12 mb-16 last:mb-0 reveal"
            >
              {/* 時刻 */}
              <div className="flex-shrink-0 md:w-1/3 md:text-right">
                <span className={`inline-block px-3 py-1 text-xs font-bold tracking-wider rounded-full mb-2 ${item.labelCls}`}>
                  {item.label}
                </span>
                <div className="text-3xl md:text-4xl font-black text-stone-200 font-serif select-none">
                  {item.time}
                </div>
              </div>
              ...
              {/* ドット (モバイル) */}
              <div className={`absolute left-0 top-1 md:hidden w-10 h-10 -translate-x-5 rounded-full flex items-center justify-center ${item.dotBg} ${item.dotText}`}>
